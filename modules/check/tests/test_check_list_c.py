...
from modules.check.check_list_c import CheckListC  # noqa: E402
...
class TestClassCheckListC(unittest.TestCase):

    @patch("modules.check.check_list_c.CheckListC._load_c_checker_lib")
    def setUp(self, mock_load):
        ...
        self.check_list = CheckListC("test.so")

    @patch("modules.check.check_list_c.ctypes.CDLL", return_value=Mock())
    def test_load_checker_lib_correct(self, mock_cdll):
        value = CheckListC._load_c_checker_lib("test.so")

        self.assertTrue(hasattr(value, "get_check_list"))
        self.assertTrue(hasattr(value, "get_api_version"))

    @patch("logging.error")
    @patch("modules.check.check_list_c.ctypes.CDLL", side_effect=Exception)
    def test_load_checker_lib_negative(self, mock_log, mock_cdll):
        self.assertRaises(Exception, CheckListC._load_c_checker_lib, "test.so")
    ...
    def test_get_check_list_from_pointers_correct(self):
        ...
        value = CheckListC._get_check_list_from_pointers(mock_pointer_list)
