...
from checkers_py import user_group_checker  # noqa: E402
from modules.check import CheckSummary, CheckMetadataPy  # noqa: E402


class TestUserGroupCheckerApiTest(unittest.TestCase):

    @patch("checkers_py.user_group_checker.are_intel_gpus_found", return_value=False)
    def test_run_positive_without_gpu(self, mocked_are_intel_gpus_found):
    ...
    @patch("checkers_py.user_group_checker.check_user_in_required_groups")
    @patch("checkers_py.user_group_checker.are_intel_gpus_found", return_value=True)
    def test_run_positive_with_gpu(self, mocked_are_intel_gpus_found, mocked_check_user_in_required_groups):
        expected = CheckSummary
...
class TestCheckUserInRequiredGroups(unittest.TestCase):
    ...
    @patch("checkers_py.user_group_checker.get_card_devices", return_value=["card"])
    @patch("checkers_py.user_group_checker.get_render_devices", return_value=["render"])
    @patch("checkers_py.user_group_checker._get_required_groups", return_value=["test1", "test2"])
    @patch("checkers_py.user_group_checker._get_user_groups", return_value=["group", "test1", "test2"])
    ...
    @patch("checkers_py.user_group_checker.get_render_devices", return_value=["render"])
    @patch("checkers_py.user_group_checker._get_required_groups", return_value=["test1", "test2"])
    @patch("checkers_py.user_group_checker._get_user_groups", return_value=["group", "test1"])
        ...
        expected = {
            ...
            "Current user is in the test2 group": {
                ...
                "Message": "Current user is not part of the test2 group, "
                           "contact the system administrator to add current user "
                           "to the test2 group."
            },
        }
    ...
    @patch("checkers_py.user_group_checker.get_render_devices", return_value=["render"])
    @patch("checkers_py.user_group_checker._get_required_groups", return_value=["test1", "test2"])
    @patch("checkers_py.user_group_checker._get_user_groups", return_value=["group", "test1", "sudo"])
    def test_check_user_in_required_groups_user_is_in_one_required_group_with_sudo(
        ...
        expected = {
            ...
            "Current user is in the test2 group": {
                ...
                "Value": "",
                "Message": "Current user is not part of the test2 group, "
                           "to add a user: 'sudo usermod -a -G test2 test_user'. "
