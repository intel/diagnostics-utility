...
def _function_prereq():
    ...
    if grep_process.wait() != 0:
        raise Exception("Cannot get information to determine if the Intel® Graphics Driver is loaded.")

    stdout, _ = grep_process.communicate()
    if not stdout.splitlines():
        raise Exception("Module i915 is not loaded, please run the following command: modprobe i915.")
...
def _check_gpu_info_path():
    ...
    if not have_dri_access:
        raise Exception("Unable to get information about initialized devices because "
...
def _get_uninitializedGPU(slots, json_node: Dict) -> int:
    have_dri_access = os.access("/sys/kernel/debug/dri/", os.R_OK)
    if not have_dri_access:
        raise Exception("Unable to get information about uninitialized devices because "
                        "the user doesn't have read access to /sys/kernel/debug/dri/.")
    hw_count = 0
    if exists('/sbin/lspci'):
        command = ['/sbin/lspci']
                ...
                if count_hw != 0:
                    gpu_detailed_info.update({
                        "Uninitialized devices": {
                            "Value": gpu_info,
                            "RetVal": "ERROR",
                            "Message": "Some GPU(s) are not initialized. To initialize GPU(s), "
                                       "please run the following command: modprobe i915."
                        }
                    })
            except Exception as error:
                gpu_detailed_info.update({
...
def get_check_list() -> List[CheckMetadataPy]:
    someCheck = CheckMetadataPy(
        ...
        type="Data",
        tags="default,gpu,sysinfo,advisor,vtune,runtime,target",
