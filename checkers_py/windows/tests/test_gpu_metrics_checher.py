...
from checkers_py.windows import gpu_metrics_checker  # noqa: E402
from modules.check import CheckSummary, CheckMetadataPy  # noqa: E402


@unittest.skipIf(sys.platform.startswith("lin"), "run on windows only")
class TestGpuMetricsCheckerApiTest(unittest.TestCase):

    @patch("checkers_py.windows.gpu_metrics_checker.parse_devices", return_value=["Device 1", "Device 2"])
    @patch("checkers_py.windows.gpu_metrics_checker.process_device")
    @patch("checkers_py.windows.gpu_metrics_checker.timeout_in_gpu_backend_check_occurred", return_value=False)  # noqa E501
    def test_run_positive(self, mocked_timeout_in_gpu_backend_check_occurred, mocked_process_device,
                          mocked_parse_devices):
    ...
    @patch("checkers_py.windows.gpu_metrics_checker.parse_devices", return_value=[])
    @patch("checkers_py.windows.gpu_metrics_checker.timeout_in_gpu_backend_check_occurred", return_value=False)  # noqa E501
    ...
    @patch("checkers_py.windows.gpu_metrics_checker.timeout_in_gpu_backend_check_occurred", return_value=True)
    ...
    def test_get_api_version_returns_str(self):
        expected = str
        actual = gpu_metrics_checker.get_api_version()
        self.assertIsInstance(actual, expected)
...
class TestProcessDevice(unittest.TestCase):
    ...
    @patch("checkers_py.windows.gpu_metrics_checker.get_max_freq_from_l0")
    @patch("checkers_py.windows.gpu_metrics_checker.get_mem_bandwidth_from_l0")
    @patch("checkers_py.windows.gpu_metrics_checker.get_pcie_bandwidth_from_l0")
    def test_process_device_known_device(self, mocked_pcie_bandwidth, mocked_mem_bandwidth, mocked_max_freq):
        device_mock = MagicMock()
        device_mock.id = "0x4905"

        gpu_metrics_checker.process_device({}, device_mock)

        mocked_pcie_bandwidth.assert_called_once()
        mocked_mem_bandwidth.assert_called_once()
        mocked_max_freq.assert_called_once()
...
class TestParseDevices(unittest.TestCase):
    ...
    @patch("checkers_py.windows.gpu_metrics_checker.is_level_zero_initialized")
...
class TestShowMetricsForUnknownDevice(unittest.TestCase):
    ...
    def test_show_metrics_for_unknown_device_unknown_metrics(self):
        expected_metrics = {
            "#0 unknown_device unknown": {
                "CheckResult": {
                    "GPU Frequency, MHz (Max/Target)": {
                        "CheckResult": "unknown/unknown",
                        "CheckStatus": "ERROR",
                        "Message": "Intel® oneAPI Level Zero driver cannot provide information about frequency.",  # noqa E501
        ...
        actual_metrics = {}
        gpu_metrics_checker.process_device(actual_metrics, device)
            ...
            '#0 known_device 0x3e98': {
                'CheckResult': {
                    'GPU Frequency, MHz (Max/Target)': {
                        ...
                        'HowToFix': 'The maximum GPU frequency: 1000, should be equal or greater than the target value: 1200.'},  # noqa E501
                    'Memory bandwidth, GB/s (Max/Target)': {
                        'CheckResult': '3/30',
                        'CheckStatus': 'FAIL',
                        'Message': 'The maximum memory bandwidth is less than the target bandwidth.',
                        'HowToFix': 'The maximum memory bandwidth: 3, should be equal or greater than the target value: 30.'},  # noqa E501
                    'PCIe bandwidth, GB/s (Max/Target)': {
                        'CheckResult': '1/10',
                        'CheckStatus': 'FAIL',
                        'Message': 'The maximum PCIe bandwidth is less than the target bandwidth.',
                        'HowToFix': 'The maximum PCIe bandwidth: 1, should be equal or greater than the target value: 10.'}},  # noqa E501
                'CheckStatus': 'WARNING', 'Message': 'For this GPU, values are not known.'}}
