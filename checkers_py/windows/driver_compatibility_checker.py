...
def _is_regression(driver_name: str, driver_version: str, cursor) -> bool:
    cursor.execute(
        ...
        ON reg.OSId = OS.Id
        WHERE com.Name = '%s' AND OS.Name = 'Windows' AND reg.Version = '%s';
...
def _is_latest_version(driver_name: str, driver_version: str, cursor) -> bool:
    cursor.execute(
        ...
        lv.OSId = OS.Id
        WHERE com.Name = '%s' and OS.Name = 'Windows';
        """ % (driver_name,)
    )
    row = cursor.fetchone()
    if row is None:
...
def get_product_versions_install(data: Dict) -> Dict:
    product_versions = {}
    try:
        toolkits = data["oneapi_toolkit_check"]["CheckResult"]["OneAPI Installed products"]["CheckResult"]  # noqa: E501
    except Exception:
        pass
    else:
        if isinstance(toolkits, dict):
            for _, components in toolkits.items():
                for name, value in components["CheckResult"].items():
                    version = value["CheckResult"]["Version"]["CheckResult"].partition("-")[0]
                    if (name in product_versions and version < product_versions[name]):
                        continue
                    product_versions[name] = version.partition("-")[0]
...
def get_check_list() -> List[CheckMetadataPy]:
    someCheck = CheckMetadataPy(
        ...
        dataReq="{\"gpu_backend_check\": 2 , \"oneapi_toolkit_check\": 2}",
        merit=60,
        timeout=5555,
