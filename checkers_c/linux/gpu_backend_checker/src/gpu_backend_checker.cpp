...
int GpuBackendChecker::PerformCheck(string &message) {
  ...
  JsonNode::AddJsonNode(drivers, "Intel® oneAPI Level Zero Driver", INFO,
                        level_zero);
  ...
  if (!lzDriverChecker->Load(message)) {
    message = "ERROR";  // todo: remove
    JsonNode::AddJsonNode(level_zero, "Driver is loaded.", ERROR, message, "",
                          0, "");
  } else {
    JsonNode::AddJsonNode(level_zero, "Driver is loaded.", PASS, 0, "");
    if (!lzDriverChecker->Initialize(message)) {
      JsonNode::AddJsonNode(level_zero, "Driver information", ERROR, message,
                            "", 0, "");
    } else {
      lzDriverChecker->GetDriverInfo(message);
      json_object *driver_info = json_tokener_parse(message.c_str());
      if (driver_info == NULL) {
        JsonNode::AddJsonNode(level_zero, "Driver information", ERROR,
                              "Unable to get driver information.", "", 0, "");
      } else {
        JsonNode::AddJsonNode(level_zero, "Driver information", INFO,
                              driver_info);
      }
    }
  }
  ...
  if (!clDriverChecker->Load(message)) {
    ...
    json_object *driver_info = json_tokener_parse(message.c_str());
    if (driver_info == NULL) {
      JsonNode::AddJsonNode(opencl, "Driver information", ERROR,
                            "Unable to get driver information.", "", 0, "");
    } else {
