...
int test(string &message)
{
	...
	if (!LZ_DriverChecker::Load(message)) {
		...
		if (!LZ_DriverChecker::Initialize(message)) {
			...
			if (driver_info == NULL) {
				JsonNode::AddJsonNode(level_zero, "Driver information", ERROR, "Unable to get driver information.", "", 0, "");
			}
			else {
	...
	if (!CL_DriverChecker::Load(message))
	{
		...
		json_object* driver_info = json_tokener_parse(message.c_str());
		if (driver_info == NULL) {
			JsonNode::AddJsonNode(opencl, "Driver information", ERROR, "Unable to get driver information.", "", 0, "");
		}
		else {
...
extern "C" EXPORT_API struct Check **get_check_list(void)
{
	return checkers;
}
