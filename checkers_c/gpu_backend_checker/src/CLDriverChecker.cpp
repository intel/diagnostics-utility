...
void CL_DriverChecker::GetDeviceInfo(cl_device_id deviceId, string& message){
    ...
    if (result == CL_SUCCESS) {
    	JsonNode::AddJsonNode(node, "Can execute OpenCL™ kernels", INFO, 2, (exec_cap & CL_EXEC_KERNEL) ? "Yes" : "No");
    	JsonNode::AddJsonNode(node, "Can execute OpenCL™ native kernels", INFO, 2, (exec_cap & CL_EXEC_NATIVE_KERNEL) ? "Yes" : "No");
	}
