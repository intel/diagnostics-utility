...
bool LZ_DriverChecker::GetLoaderVersion(string &message)
{
	...
	if (retval != 0) {
		message = "Cannot obtain installed level-zero package.";
		return false;
	}
...
void LZ_DriverChecker::GetDriverInfo(string &message)
{
	...
	if (user_driver_index != nullptr) {
		try {
			uint32_t default_idx = stoi(user_driver_index);
			if (default_idx >= uint32_t(0) && default_idx < driverCount)
...
static void add_clock_rate(json_object *device, ze_device_handle_t device_handler, ze_device_properties_t device_properties)
{
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "Device maximum frequency, MHz", ERROR, "Can't get number of frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, NULL)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device current frequency, MHz", ERROR, "Can't get number of frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, NULL)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device minimum frequency, MHz", ERROR, "Can't get number of frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, NULL)", 1, "unknown");
		return;
	}
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "Device maximum frequency, MHz", ERROR, "Can't get frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, pFreqHandles)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device current frequency, MHz", ERROR, "Can't get frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, pFreqHandles)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device minimum frequency, MHz", ERROR, "Can't get frequency domains", "zesDeviceEnumFrequencyDomains(hSysmanDevice, &numFreqDomains, pFreqHandles)", 1, "unknown");
		delete[] pFreqHandles;
		return;
	}
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "Device maximum frequency, MHz", ERROR, "Can't get frequency properties", "zesFrequencyGetProperties(pFreqHandle, &pFreqProperties)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device current frequency, MHz", ERROR, "Can't get frequency properties", "zesFrequencyGetProperties(pFreqHandle, &pFreqProperties)", 1, "unknown");
		JsonNode::AddJsonNode(device, "Device minimum frequency, MHz", ERROR, "Can't get frequency properties", "zesFrequencyGetProperties(pFreqHandle, &pFreqProperties)", 1, "unknown");
		delete[] pFreqHandles;
		return;
	}
...
static void add_memory_bandwidth(json_object *device, ze_device_handle_t device_handler)
{
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "Memory bandwidth, GB/s", ERROR, "Can't get number of memory domains", "zesDeviceEnumMemoryModules(hSysmanDevice, &numMemDomains, NULL)", 1, "unknown");
		return;
	}
	if (numMemDomains == 0) {
		JsonNode::AddJsonNode(device, "Memory bandwidth, GB/s", ERROR, "Driver can't get memory bandwidth", "zesDeviceEnumMemoryModules(hSysmanDevice, &numMemDomains, NULL)", 1, "unknown");
		return;
	}
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "Memory bandwidth, GB/s", ERROR, "Driver can't get memory bandwidth", "zesDeviceEnumMemoryModules(hSysmanDevice, &numMemDomains, pMemHandles)", 1, "unknown");
		delete[] pMemHandles;
		return;
	}
...
static void add_pci_bandwidth(json_object *device, ze_device_handle_t device_handler)
{
	...
	if (result != ZE_RESULT_SUCCESS) {
		JsonNode::AddJsonNode(device, "PCIe bandwidth, GB/s", ERROR, "Can't get PCIe information", "zesDevicePciGetProperties(hSysmanDevice, &pPCIProps)", 1, "unknown");
		return;
	}
	int raw_bandwidth = pPCIProps.maxSpeed.maxBandwidth;
	if (raw_bandwidth == -1) {
		JsonNode::AddJsonNode(device, "PCIe bandwidth, GB/s", ERROR, "Driver can't get PCIe bandwidth", "zesDevicePciGetProperties(hSysmanDevice, &pPCIProps)", 1, "unknown");
		return;
	}
	double bandwidth = double(raw_bandwidth) / double(1024 * 1024 * 1024); // convert B/s to GB/s
